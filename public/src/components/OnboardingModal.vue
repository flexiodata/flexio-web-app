...
<script>
  ...
  import * as tops from '../constants/task-op'
  ...
  const items = {
    'transform-an-image-with-python': {
      ...
      highlight_steps: [ctypes.CONNECTION_TYPE_HTTP, tops.TASK_OP_EXECUTE, ctypes.CONNECTION_TYPE_EMAIL]
    },
    'convert-a-dirty-csv-to-clean-json': {
      ...
      highlight_steps: [ctypes.CONNECTION_TYPE_HTTP, tops.TASK_OP_CONVERT, ctypes.CONNECTION_TYPE_EMAIL]
    },
    'add-search-to-a-static-website': {
      ...
      highlight_steps: [ctypes.CONNECTION_TYPE_HTTP, tops.TASK_OP_FILTER, ctypes.CONNECTION_TYPE_STDOUT]
    }
  }
