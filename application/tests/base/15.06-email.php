<?php
/**
 *
 * Copyright (c) 2016, Gold Prairie, Inc.  All rights reserveG.
 *
 * Project:  Flex.io App
 * Author:   Aaron L. Williams
 * Created:  2016-06-28
 *
 * @package flexio
 * @subpackage Tests
 */


declare(strict_types=1);
namespace Flexio\Tests;


function getTestEmailContents($filename)
{
    // loads a test email stream from the php parsing library;
    // TODO: remove external library dependency on these tests

    $testfilepath = dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'library' . DIRECTORY_SEPARATOR . 'mailmimeparser' . DIRECTORY_SEPARATOR . 'tests' . DIRECTORY_SEPARATOR . '_data' . DIRECTORY_SEPARATOR . 'emails';
    $testfile = $testfilepath . DIRECTORY_SEPARATOR . $filename;
    return file_get_contents($testfile);
}

function getTestEmailFile($filename)
{
    $testfilepath = dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'library' . DIRECTORY_SEPARATOR . 'mailmimeparser' . DIRECTORY_SEPARATOR . 'tests' . DIRECTORY_SEPARATOR . '_data' . DIRECTORY_SEPARATOR . 'emails';
    $testfile = $testfilepath . DIRECTORY_SEPARATOR . $filename;
    return $testfile;
}


class Test
{
    public function run(&$results)
    {
        // TODO: fix bad tests that are commented out

        // TEST: content parsing; empty input

        // BEGIN TEST
        $email = \Flexio\Base\Email::parseText('');
        $actual = get_class($email);
        $expected = 'Flexio\Base\Email';
        \Flexio\Tests\Check::assertString('A.1', '\Flexio\Base\Email::parseText(); empty input', $actual, $expected, $results);



        // TEST: content parsing; "from" information

        // BEGIN TEST
        $content = getTestEmailContents('m0001.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.1', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0002.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.2', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0003.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.3', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0004.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.4', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0005.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.5', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0006.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.6', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0007.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.7', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0008.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.8', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0009.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.9', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0010.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.10', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0011.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.11', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0012.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.12', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0013.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.13', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0014.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.14', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0015.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.15', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0016.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.16', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0017.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.17', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0018.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.18', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0019.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.19', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0020.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getFrom();
        $expected = '["Doug Sauder <doug@example.com>"]';
        \Flexio\Tests\Check::assertArray('B.20', '\Flexio\Base\Email::parseText(); get the "from" addresses', $actual, $expected, $results);



        // TEST: content parsing; "to" information

        // BEGIN TEST
        $content = getTestEmailContents('m0001.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.1', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0002.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.2', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0003.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.3', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0004.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.4', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0005.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.5', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0006.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>","J\u00fcrgen Schm\u00fcrgen <schmuergen@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.6', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0007.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Heinz M\u00fcller <mueller@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.7', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0008.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Heinz M\u00fcller <mueller@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.8', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0009.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Heinz M\u00fcller <mueller@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.9', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0010.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Heinz M\u00fcller <mueller@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.10', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0011.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Heinz M\u00fcller <mueller@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.11', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0012.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.12', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0013.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.13', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0014.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.14', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0015.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.15', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0016.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.16', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0017.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.17', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0018.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.18', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0019.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.19', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0020.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getTo();
        $expected = '["Joe Blow <jblow@example.com>"]';
        \Flexio\Tests\Check::assertArray('C.20', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);



        // TEST: content parsing; "cc" information

        // BEGIN TEST
        $content = getSampleEmailWithAttachment();
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getCC();
        $expected = '["you2 <you2@flex.io>"]';
        \Flexio\Tests\Check::assertArray('D.1', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);



        // TEST: content parsing; "bcc" information

        // BEGIN TEST
        $content = getSampleEmailWithAttachment();
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getBCC();
        $expected = '["you3 <you3@flex.io>", "you4 <you4@flex.io>"]';
        \Flexio\Tests\Check::assertArray('E.1', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);



        // TEST: content parsing; "reply-to" information

        // BEGIN TEST
        $content = getSampleEmailWithAttachment();
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getReplyTo();
        $expected = '["You4 <You4@flex.io>"]';
        \Flexio\Tests\Check::assertArray('F.1', '\Flexio\Base\Email::parseText(); get the "to" addresses', $actual, $expected, $results);



        // TEST: content parsing; "subject" information

        // BEGIN TEST
        $content = getTestEmailContents('m0001.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = "Die Hasen und die Frösche (Microsoft Outlook 00)";
        \Flexio\Tests\Check::assertString('G.1', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0002.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.2', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0003.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.3', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0004.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.4', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0005.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.5', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0006.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.6', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0007.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.7', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0008.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.8', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0009.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.9', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0010.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Die Hasen und die Frösche (Microsoft Outlook 00)';
        \Flexio\Tests\Check::assertString('G.10', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0011.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.11', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0012.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.12', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0013.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.13', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0014.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.14', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0015.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.15', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0016.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.16', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0017.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.17', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0018.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.18', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0019.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.19', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0020.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = $email->getSubject();
        $expected = 'Test message from Microsoft Outlook 00';
        \Flexio\Tests\Check::assertString('G.20', '\Flexio\Base\Email::parseText(); get the "subject"', $actual, $expected, $results);



        // TEST: content parsing; "message text" information

        // BEGIN TEST
        $content = getTestEmailContents('m0001.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.1', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0002.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.2', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0003.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.3', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0004.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.4', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0005.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.5', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0006.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.6', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0007.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.7', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0008.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.8', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0009.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.9', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0010.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 4;
        \Flexio\Tests\Check::assertString('H.10', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0011.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.11', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0012.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 0;
        \Flexio\Tests\Check::assertString('H.12', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0013.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 0;
        \Flexio\Tests\Check::assertString('H.13', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0014.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.14', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0015.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.15', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0016.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.16', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0017.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.17', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0018.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.18', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0019.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.19', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0020.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageText(),".");
        $expected = 7;
        \Flexio\Tests\Check::assertString('H.20', '\Flexio\Base\Email::parseText(); get the message text', $actual, $expected, $results);



        // TEST: content parsing; "message html" information

        // BEGIN TEST
        $content = getTestEmailContents('m0001.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.1', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0002.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.2', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0003.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.3', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0004.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.4', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0005.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.5', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0006.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.6', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0007.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.7', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0008.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.8', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0009.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.9', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0010.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.10', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0011.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.11', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0012.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.12', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0013.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.13', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0014.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 19;
        \Flexio\Tests\Check::assertNumber('I.14', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0015.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 19;
        \Flexio\Tests\Check::assertNumber('I.15', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0016.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 35;
        \Flexio\Tests\Check::assertNumber('I.16', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0017.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 28;
        \Flexio\Tests\Check::assertNumber('I.17', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0018.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 0;
        \Flexio\Tests\Check::assertNumber('I.18', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0019.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 19;
        \Flexio\Tests\Check::assertNumber('I.19', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);

        // BEGIN TEST
        $content = getTestEmailContents('m0020.txt');
        $email = \Flexio\Base\Email::parseText($content);
        $actual = substr_count($email->getMessageHtml(),"<");
        $expected = 19;
        \Flexio\Tests\Check::assertNumber('I.20', '\Flexio\Base\Email::parseText(); get the message html', $actual, $expected, $results);
    }
}


function getSampleEmailWithAttachment()
{
    // TODO: factor out into test file

    // following is the raw text of an email with the following values:
    // from: "me" <me@flex.io>
    // to: "you1" <you1@flex.io>
    // cc: "you2" <you2@flex.io>
    // bcc: "you3" <you3@flex.io>
    // reply-To: "you4" <you4@flex.io>
    // subject: Attachment Test
    // body: This is an email with a CSV attachment.
    // attachment: Vend_mast.csv

    $data = <<<EOD
From me@flex.io  Wed Aug  3 23:52:22 2016
Return-Path: <me@flex.io>
X-Original-To: you1@flex.io
Delivered-To: you1@flex.io
Received: from NAM02-SN1-obH.outbounG.protection.outlook.com (mail-sn1nam02on0052.outbounG.protection.outlook.com [104.47.36.52])
        by pipes.flex.io (Postfix) with ESMTPS id 6DD0B805B6
        for <you1@flex.io>; Wed,  3 Aug 2016 23:52:22 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=flexio.onmicrosoft.com; s=selector1-flex-io;
 h=From:Date:Subject:Message-ID:Content-Type:MIME-Version;
 bh=OD6P+69gssDuX8P6hOgBJSoZR5GS1wuiHS6WYAJEK2A=;
 b=ByLutEEEWeYNHWFvZGz114atu0pc6TjCR5c5vS217Lxnj+veVGwxSApkktcoyKsF8bEz45P6K1ff14ukujR6OnLYWLTLbrAFTnJy+W6r0tUlTR8ymTukdDZkrEwIUPN9m2p5pokhkjP3BsvQrpl5ucQFQgHWYNVqNEWMpW+uSBk=
Received: from BN6PR12MB1233.namprd12.proG.outlook.com (10.168.227.19) by
 BN6PR12MB1233.namprd12.proG.outlook.com (10.168.227.19) with Microsoft SMTP
 Server (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384_P384) id
 15.1.549.15; Wed, 3 Aug 2016 23:53:19 +0000
Received: from BN6PR12MB1233.namprd12.proG.outlook.com ([10.168.227.19]) by
 BN6PR12MB1233.namprd12.proG.outlook.com ([10.168.227.19]) with mapi id
 15.01.0549.022; Wed, 3 Aug 2016 23:53:19 +0000
From: "me" <me@flex.io>
To: "you1" <you1@flex.io>
Cc: "you2" <you2@flex.io>
Bcc: you3 <you3@flex.io>, you4 <you4@flex.io>
Reply-To: You4 <You4@flex.io>
Subject: Attachment Test
Thread-Topic: Attachment Test
Thread-Index: AdHt4e012cGejNcTRqasix1Acbd9mA==
Date: Wed, 3 Aug 2016 23:53:18 +0000
Message-ID: <BN6PR12MB1233850235FFD6CEF9272D10B6060@BN6PR12MB1233.namprd12.proG.outlook.com>
Accept-Language: en-US
Content-Language: en-US
X-MS-Has-Attach: yes
X-MS-TNEF-Correlator:
authentication-results: spf=none (sender IP is ) smtp.mailfrom=test@flex.io;
x-originating-ip: [168.93.116.250]
x-ms-office365-filtering-correlation-id: 8ae1131a-6f87-498c-b862-08d3bbf95838
x-microsoft-exchange-diagnostics: 1;BN6PR12MB1233;6:;5:jkqqPbWAI53qk0XAFktwK4MX/fxjNJy8/OZL7wd7fRkWZQlOXSd/Op14agKyEZJKWUI99kB0DQLaGx8KMj/pLhPWZKU1xFkOsMZttqDwGSn5LWCSmPgnTT90mi7lQuukk76RkIGNVfoCQjsc9S4N7Q==;24:TL9klAIbAVn9SjfKlkHNjRL7i6j4No/+T4WW90cDQkNXUd66nKZUzlopA4Kd/AUOFpgPRLPfEbrjhSUpAl4Ljvv0LvvTdCiOkJQY4V6tZkg=;7:
x-microsoft-antispam: UriScan:;BCL:0;PCL:0;RULEID:;SRVR:BN6PR12MB1233;
x-microsoft-antispam-prvs: <BN6PR12MB1233997D941C418AC2E05B28B6060@BN6PR12MB1233.namprd12.proG.outlook.com>
x-exchange-antispam-report-test: UriScan:(21748063052155);
x-exchange-antispam-report-cfa-test: BCL:0;PCL:0;RULEID:(102415321)(6040130)(601004)(2401047)(8121501046)(5005006)(3002001)(10201501046)(6041072)(6043046);SRVR:BN6PR12MB1233;BCL:0;PCL:0;RULEID:;SRVR:BN6PR12MB1233;
x-forefront-prvs: 00235A1EEF
x-forefront-antispam-report: SFV:NSPM;SFS:(10009020)(6009001)(7916002)(199003)(189002)(7696003)(2351001)(229853001)(101416001)(97736004)(7846002)(7736002)(19625215002)(68736007)(77096005)(74316002)(76576001)(19580395003)(588024002)(87936001)(110136002)(107886002)(4270600005)(9686002)(221733001)(1240700005)(5890100001)(15975445007)(54356999)(33656002)(586003)(790700001)(558084003)(3480700004)(92566002)(5002640100001)(6116002)(66066001)(102836003)(81166006)(81156014)(8676002)(106356001)(3660700001)(1730700003)(3280700002)(3846002)(16236675004)(10400500002)(50986999)(8936002)(122556002)(11100500001)(74482002)(189998001)(99936001)(2906002)(99286002)(2900100001)(86362001)(2501003)(19300405004)(105586002)(450100001);DIR:OUT;SFP:1101;SCL:1;SRVR:BN6PR12MB1233;H:BN6PR12MB1233.namprd12.proG.outlook.com;FPR:;SPF:None;PTR:InfoNoRecords;A:1;MX:1;LANG:en;
received-spf: None (protection.outlook.com: flex.io does not designate
 permitted sender hosts)
spamdiagnosticoutput: 1:99
spamdiagnosticmetadata: NSPM
Content-Type: multipart/mixed;
        boundary="_004_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_"
MIME-Version: 1.0
X-OriginatorOrg: flex.io
X-MS-Exchange-CrossTenant-originalarrivaltime: 03 Aug 2016 23:53:18.6351
 (UTC)
X-MS-Exchange-CrossTenant-fromentityheader: Hosted
X-MS-Exchange-CrossTenant-id: ea643978-1238-48ab-9676-b53394c6d27b
X-MS-Exchange-Transport-CrossTenantHeadersStamped: BN6PR12MB1233

--_004_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_
Content-Type: multipart/alternative;
        boundary="_000_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_"

--_000_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_
Content-Type: text/plain; charset="us-ascii"
Content-Transfer-Encoding: quoted-printable


This is an email with a CSV attachment.



--_000_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_
Content-Type: text/html; charset="us-ascii"
Content-Transfer-Encoding: quoted-printable

<html xmlns:v=3D"urn:schemas-microsoft-com:vml" xmlns:o=3D"urn:schemas-micr=
osoft-com:office:office" xmlns:w=3D"urn:schemas-microsoft-com:office:word" =
xmlns:m=3D"http://schemas.microsoft.com/office/2004/12/omml" xmlns=3D"http:=
//www.w3.org/TR/REC-html40">
<head>
<meta http-equiv=3D"Content-Type" content=3D"text/html; charset=3Dus-ascii"=
>
<meta name=3D"Generator" content=3D"Microsoft Word 15 (filtered medium)">
<style><!--
/* Font Definitions */
@font-face
        {font-family:"Cambria Math";
        panose-1:2 4 5 3 5 4 6 3 2 4;}
@font-face
        {font-family:Calibri;
        panose-1:2 15 5 2 2 2 4 3 2 4;}
/* Style Definitions */
p.MsoNormal, li.MsoNormal, div.MsoNormal
        {margin:0in;
        margin-bottom:.0001pt;
        font-size:11.0pt;
        font-family:"Calibri",sans-serif;}
a:link, span.MsoHyperlink
        {mso-style-priority:99;
        color:#0563C1;
        text-decoration:underline;}
a:visited, span.MsoHyperlinkFollowed
        {mso-style-priority:99;
        color:#954F72;
        text-decoration:underline;}
span.EmailStyle17
        {mso-style-type:personal-compose;
        font-family:"Calibri",sans-serif;
        color:windowtext;}
.MsoChpDefault
        {mso-style-type:export-only;
        font-family:"Calibri",sans-serif;}
@page WordSection1
        {size:8.5in 11.0in;
        margin:1.0in 1.0in 1.0in 1.0in;}
div.WordSection1
        {page:WordSection1;}
--></style><!--[if gte mso 9]><xml>
<o:shapedefaults v:ext=3D"edit" spidmax=3D"1026" />
</xml><![endif]--><!--[if gte mso 9]><xml>
<o:shapelayout v:ext=3D"edit">
<o:idmap v:ext=3D"edit" data=3D"1" />
</o:shapelayout></xml><![endif]-->
</head>
<body lang=3D"EN-US" link=3D"#0563C1" vlink=3D"#954F72">
<div class=3D"WordSection1">
<p class=3D"MsoNormal"><o:p>&nbsp;</o:p></p>
<p class=3D"MsoNormal">This is an email with a CSV attachment.<o:p></o:p></=
p>
<p class=3D"MsoNormal"><o:p>&nbsp;</o:p></p>
<p class=3D"MsoNormal"><o:p>&nbsp;</o:p></p>
</div>
</body>
</html>

--_000_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_--

--_004_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_
Content-Type: application/octet-stream; name="Vend_mast.csv"
Content-Description: Vend_mast.csv
Content-Disposition: attachment; filename="Vend_mast.csv"; size=8695;
        creation-date="Thu, 04 Feb 2016 16:24:49 GMT";
        modification-date="Thu, 04 Feb 2016 16:24:33 GMT"
Content-Transfer-Encoding: base64



--_004_BN6PR12MB1233850235FFD6CEF9272D10B6060BN6PR12MB1233namp_--
EOD;

    return $data;
}
