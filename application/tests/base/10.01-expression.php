...
class Test
{
    public function run(&$results)
    {
        ...
        TestCheck::assertString('A.1', 'Expression; non-string input',  $actual, $expected, $results);

        $actual = TestUtil::evalExpression(true);
        $expected = TestError::ERROR_BAD_PARSE;
        TestCheck::assertString('A.2', 'Expression; non-string input',  $actual, $expected, $results);

        $actual = TestUtil::evalExpression(1);
        $expected = TestError::ERROR_BAD_PARSE;
        TestCheck::assertString('A.3', 'Expression; non-string input',  $actual, $expected, $results);
        ...
        $expected = TestError::ERROR_BAD_PARSE;
        TestCheck::assertString('A.6', 'Expression; empty string',  $actual, $expected, $results);
    }
}
