...
class Test
{
    public function run(&$results)
    {
        ...
        $actual = \Flexio\Base\Eid::isValid($eid);
        $expected = true;
        \Flexio\Tests\Check::assertBoolean('B.1', '\Model::create(); make sure valid eid is returned when project with name is created',  $actual, $expected, $results);
        ...
        $eid_first_time_creation = \Flexio\Tests\Util::getModel()->create(\Model::TYPE_PROJECT, $info);
        $eid_second_time_creation = \Flexio\Tests\Util::getModel()->create(\Model::TYPE_PROJECT, $info);
        $actual = (\Flexio\Base\Eid::isValid($eid_first_time_creation) && \Flexio\Base\Eid::isValid($eid_second_time_creation));
        $expected = true;
        \Flexio\Tests\Check::assertBoolean('B.2', '\Model::create(); allow multiple projects with the same name',  $actual, $expected, $results);
    }
}
