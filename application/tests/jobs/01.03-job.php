...
class Test
{
    public function run(&$results)
    {
        ...
        TestCheck::assertString('A.9', '\Flexio\Jobs\Fail::create()',  $actual, $expected, $results);

        // BEGIN TEST
        $task = \Flexio\Jobs\Filter::create();
        $actual = ($task !== false ? $task->getType() : false);
        $expected = \Flexio\Jobs\Filter::MIME_TYPE;
        TestCheck::assertString('A.10', '\Flexio\Jobs\Filter::create()',  $actual, $expected, $results);
        ...
        TestCheck::assertString('A.21', '\Flexio\Jobs\RenameColumn::create()',  $actual, $expected, $results);
        ...
        TestCheck::assertString('A.23', '\Flexio\Jobs\Search::create()',  $actual, $expected, $results);

        // BEGIN TEST
        $task = \Flexio\Jobs\Select::create();
        $actual = ($task !== false ? $task->getType() : false);
        $expected = \Flexio\Jobs\Select::MIME_TYPE;
        TestCheck::assertString('A.24', '\Flexio\Jobs\Select::create()',  $actual, $expected, $results);
