...
/**
 *
 * Copyright (c) 2014, Flex Research LLC. All rights reserved.
 *
 * Project:  Flex.io App
...
class Api
{
    ...
    private static $endpoints = array(
        ...
        'POS /:teamid/members/leave'                  => '\Flexio\Api\TeamMember::processleave',
    ...
    public static function request(\Flexio\System\FrameworkRequest $server_request) : void
    {
        ...
        if (IS_DEBUG() && (strpos($request_http_origin,"://localhost:") !== false) || $request_http_host == 'localhost')
        {
            ...
            header('Access-Control-Allow-Headers: authorization, origin, x-csrftoken, content-type, accept, x-flexio-source, x-flexio-caller'); // note that '*' is not valid for Access-Control-Allow-Headers
        }
        else
        {
            if (substr($request_http_host, 0, 4) == 'api.' && substr($request_http_host, -8) == '.flex.io')
            {
                ...
                header('Access-Control-Allow-Methods: GET, PUT, POST, DELETE, PATCH, HEAD');
                header('Access-Control-Allow-Headers: authorization, content-type, x-flexio-source, x-flexio-caller');
            }
